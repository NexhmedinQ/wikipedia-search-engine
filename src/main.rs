...
use log4rs::{append::console::ConsoleAppender, config::{Appender, Logger, Root}, encode::pattern::PatternEncoder, Config};
use log::LevelFilter;
pub mod crawler;
pub mod indexer;
...
fn main() {
    ...
    .encoder(Box::new(PatternEncoder::new("{d(%Y-%m-%d %H:%M:%S)} | {h({l})} | {f}:{L} - {m}{n}")))
    .build();
